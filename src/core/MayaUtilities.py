...
def ConfigureCtrlForJnt(jnt, ctrlName, doContraint=True):
    ...
    if doContraint:#match transform
        mc.orientConstraint(ctrlName, jnt) #orient Constraint

    return ctrlName, ctrlGrpName

def CreatePlusController(namePrefix, size=10):
    ctrlName =f"ac_{namePrefix}"
    cmd = (f"curve -n {ctrlName} -d 1 -p -1 0 -1 -p -1 0 -3 -p 1 0 -3 -p 1 0 -1 -p 3 0 -1 -p 3 0 1 -p 1 0 1 -p 1 0 3 -p -1 0 3 -p -1 0 1 -p -3 0 1 -p -3 0 -1 -p -1 0 -1 -k 0 -k 1 -k 2 -k 3 -k 4 -k 5 -k 6 -k 7 -k 8 -k 9 -k 10 -k 11 -k 12 ;")
    ml.eval(cmd)
...
##SCALE
    ...
    setCurvelLineWidth(ctrlName,2)
    return ctrlName
#how to lock and hide attrubets in maya
...
def setCurvelLineWidth(curve, newWidth):
